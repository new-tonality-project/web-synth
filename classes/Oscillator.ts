...
export class Oscillator {
  ...
  public play(pitch: number, time = this._audioContext.currentTime) {
    if (this._state === "used")
      throw new Error(`Attempting to start oscillator with 'used' state`);

    const frequency = this._rate * pitch;
    if (frequency > 20000 || frequency < 20)
      throw new Error(`Oscillator frequency="${frequency}" is out of range`);
  ...
  private buildOscNode() {
    ...
    const wave = this._audioContext.createPeriodicWave(
      [0, this._phase],
      [0, 1 - this._phase]
    );
