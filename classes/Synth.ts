...
export class AdditiveSynth {
  ...
  public constructor({
    ...
    adsr,
    audioContext,
  }: AdditiveSynthConstructorOptions) {
  ...
  public async generateSample({
    ...
    const sample = new Promise<AudioBuffer>((resolve) => {
      ...
      const allPartials = this._spectrum
        .flatMap((layer) => layer.partials)
        .sort((a, b) => a.rate - b.rate);

      for (let p = 0; p < length; p++) {
        for (const partial of allPartials) {
          const omega = 2 * Math.PI * partial.rate * fundamental;

          channelData[p]! +=
            Math.sin((p / sampleRate) * omega + (partial.phase || 0)) *
            partial.amplitude;
        }
